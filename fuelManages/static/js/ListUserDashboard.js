...
function checkLostCard(id, avtUrl, rfidCode) {
    var listMessages = firebase.database().ref('Card drop');
...
function getIdRemove(id) {
    ...
    if (result) {
        firebase.database().ref("Account/" + idUser[id.substr(id.length - 1) - 1]).remove();
    }
}
...
function removeCardLost(id, UId, rfid) {
    var result = confirm("Want to delete?");
    if (result) {
        firebase.database().ref('Card drop/' + UId).remove()
        firebase.database().ref("Account/" + UId + "/RFID").remove()
        firebase.database().ref("Account/" + UId + "/PINCODE").remove()
        firebase.database().ref("Account/" + UId + "/date_creat_rfid").remove()
    }
}
